...
namespace API.Data
{
    public class MessageRepository : IMessageRepository
    {
        ...
        {
            return await _context.Groups.Include(c => c.Connections).FirstOrDefaultAsync(g => g.Name == groupName);
        ...
        {
            var a = _mapper.ConfigurationProvider;
            var messages = await _context.Messages
                .Include(u => u.Sender).ThenInclude(p => p.Photos)
                .Include(u => u.Recipient)
                .Where(m => m.Recipient.UserName == recipientUsername && m.Sender.UserName == currentUsername && m.SenderDeleted == false
